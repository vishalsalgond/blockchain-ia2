			...
			App.contracts.RetailSupplyChain.deployed().then(function (instance) {
				...
				manufacInstance.addProduct.sendTransaction(productID, product, brand, locationM).then(function (hash) {
					web3.eth.getTransaction(hash, function (err, tx) {
						...
						tr += "<td>" + tx.hash + "</td>";
						tr += "<td>" + "New Product Created Successfully" + "</td>";
						tr += "</tr>";
						t += tr;
	...
	handleDistributer: function (event) {
		...
		web3.eth.getAccounts(function (error, accounts) {
			...
			App.contracts.RetailSupplyChain.deployed().then(function (instance) {
				...
				distInstance.shipProduct.sendTransaction(productID, agency, destAddr, 0, retailerAddress).then(function (hash) {
	...
	handleRetailTracking: function (event) {
		...
		web3.eth.getAccounts(function (error, accounts) {
			...
			App.contracts.RetailSupplyChain.deployed().then(function (instance) {
				...
				newRow.innerHTML = "<th id=\"test\">Owner</th><th>Location</th><th>Description</th><th>Time</th>";
				var tr;
				console.log(result);
