...
contract RetailSupplyChain {
    ...
    mapping(uint256 => ShipmentDetails) shipmentInfo;
    mapping(uint256 => TrackDetails[]) trackInfoMap;
    ...
    function addProduct(
        ...
        TrackDetails memory track = TrackDetails(
            "Manufacturing Unit",
            location,
    ...
    function listAllTrackInfo(uint256 productID)
        public
        returns (bytes32[] memory)
    {
        ...
        for (
            iterator = 0;
            iterator < trackInfoMap[productID].length;
            iterator++
        ) {
            trackByteArray.push(stringToBytes32(trackDetails[iterator].owner));
            trackByteArray.push(
                stringToBytes32(trackDetails[iterator].location)
            );
            trackByteArray.push(
                stringToBytes32(trackDetails[iterator].description)
            );
    ...
    function getTrackInfo(uint256 productID, uint256 index)
        public
        view
        returns (
            string memory owner,
    ...
    function getSellerAddress(uint256 _productID)
        public
        view
        returns (address)
    {
